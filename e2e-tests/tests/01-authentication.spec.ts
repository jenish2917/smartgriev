 ...
 *    Error: "Test timeout of 60000ms exceeded at waitForLoadState('domcontentloaded')"
 *    
 *    WHY IT HAPPENS:
 ...
 *      Find: await page.waitForLoadState('domcontentloaded');
...
test.describe('User Authentication Flow', () => {
  ...
  test('should complete user signup flow with OTP verification', async ({ page }) => {
    ...
    try {
      ...
      console.log('✅ Page loaded successfully (DOM ready)');
      // Wait for form to be visible - be more lenient with timeout
      await page.waitForSelector('input[name="email"], input[name="firstName"]', { timeout: 20000 });
      console.log('✅ Form elements are visible and ready');
    } catch (error) {
      console.error('❌ Page load timeout - DOM not ready or form not found');
      console.error('This usually means:');
      console.error('  1. Frontend not running on http://localhost:3000');
      console.error('  2. React component failed to mount');
      console.error('  3. JavaScript errors in console');
      console.error('Check DevTools Console and Network tabs');
      throw error;
    }
